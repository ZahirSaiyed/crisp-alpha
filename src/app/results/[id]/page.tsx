  ...
  return (
    <main className="min-h-screen bg-[var(--bg-warm)] text-[var(--ink)]">
      ...
      <header className="mx-auto max-w-5xl px-6 py-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-[var(--ink)] tracking-tight" style={{ fontFamily: 'var(--font-display)' }}>Your Results</h1>
          <p className="text-sm text-[var(--ink-light)] mt-1">Insights from your speaking practice</p>
        </div>
        {/* Coach functionality disabled for now */}
        {false && (
          <button type="button" onClick={() => router.push(`/coach/${id}`)} className="px-3 py-2 rounded-full border border-[var(--muted-2)] bg-white text-[var(--ink)] hover:border-[var(--ink-light)] transition-colors">
