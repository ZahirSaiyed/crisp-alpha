...
export default function PromptSwiper({ prompts, onSelect }: { prompts: Prompt[]; onSelect: (p: Prompt) => void }) {
  ...
  const primaryAction = useCallback(() => {
    if (isRecording) {
      ...
      setIsRecording(true);
      window.dispatchEvent(new Event("app:begin-recording"));
      choose(current);
    }
  }, [isRecording, current, choose]);
  ...
  useEffect(() => {
    function onStop() { setIsRecording(false); }
    window.addEventListener("app:stop-recording", onStop as EventListener);
    return () => window.removeEventListener("app:stop-recording", onStop as EventListener);
  }, []);
    ...
    >
      ...
      >
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          {isRecording && (
            <div className="ml-auto inline-flex items-center gap-2 text-[12px] text-red-600">
        ...
        <div className="mt-5 flex items-center gap-3">
          <m.button
            ...
            onClick={onPrimary}
          >
            {isRecording ? "Stop" : "Begin recording"}
          </m.button>
